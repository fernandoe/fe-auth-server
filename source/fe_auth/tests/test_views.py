...
class TestTokenIntrospectionViews(TestCase):
    def setUp(self):
        ...
        self.user_with_entity = UserFactory()

        self.application_with_user_with_entity = Application.objects.create(
            name="Test Application",
            redirect_uris="http://localhost http://example.com http://example.org",
            user=self.user_with_entity,
            client_type=Application.CLIENT_CONFIDENTIAL,
            authorization_grant_type=Application.GRANT_AUTHORIZATION_CODE,
        )

        self.token_with_user_with_entity = AccessToken.objects.create(
            user=self.user_with_entity, token="token_with_user_with_entity",
            application=self.application_with_user_with_entity,
            expires=timezone.now() + datetime.timedelta(days=1),
            scope="read write dolphin"
        )
    ...
    def test_view_forbidden(self):
        response = self.client.get(reverse("introspect"))
    ...
    def test_view_get_valid_token(self):
        ...
        response = self.client.get(reverse("introspect"), {"token": self.valid_token.token}, **auth_headers)
    ...
    def test_view_post_valid_token(self):
        ...
        response = self.client.post(reverse("introspect"), {"token": self.valid_token.token},
                                    **auth_headers)
    ...
    def test_view_with_entity(self):
        ...
        response = self.client.get(reverse("introspect"), {"token": self.token_with_user_with_entity.token}, **auth_headers)
        ...
        self.assertDictEqual(content, {
            "active": True,
            "scope": self.token_with_user_with_entity.scope,
            "client_id": self.token_with_user_with_entity.application.client_id,
            "username": self.token_with_user_with_entity.user.get_username(),
            "exp": int(calendar.timegm(self.token_with_user_with_entity.expires.timetuple())),
            "entity": str(self.token_with_user_with_entity.user.entity.uuid)
        })
